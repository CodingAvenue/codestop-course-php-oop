...
class Decrement extends Rule implements RuleInterface
{
    const POST_CLASS_ = '\PhpParser\Node\Expr\PostDec';
    const PRE_CLASS_ = '\PhpParser\Node\Expr\PreDec';
    ...
    public function checkType()
    {
        if (isset($this->filter['type'])) {
            ...
            if ($type !== 'pre' && $type !== 'post') {
                throw new \Exception("Unknown type {$this->filter['type']}. Supported type is 'pre' and 'post'");
            }
        }
    }
